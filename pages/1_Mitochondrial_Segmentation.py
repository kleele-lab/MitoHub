...
class MakeCropsDetectThem:
    ...
    def get_crops_xy(
        ...
        if show:
            #plt.show()
            plt.savefig(str(output_dir_path)+'_'+str(current_dateTime)+'/patches_'+str(basename(input_image)), 
...
class CropElement:
    ...
    def calculate_inference(self, model, imgsz=640, conf=0.35, iou=0.7, segment=True, classes_list=None, memory_optimize=False, extra_args=None):
...
def visualize_results(
    ...
    if return_image_array:
        ...
        if axis_off:
            plt.axis('off')
        #plt.show()
        plt.savefig(str(output_dir_path)+'_'+str(current_dateTime)+'/segment_mask_'+str(basename(input_image)), 
                    dpi=500, bbox_inches='tight', pad_inches = 0)
        
        plt.imshow(labeled_image_mask)
        plt.savefig(str(output_dir_path)+'_'+str(current_dateTime)+'/mask_'+str(basename(input_image)), 
                    dpi=500, bbox_inches='tight', pad_inches = 0)

        tifffile.imwrite(str(output_dir_path)+'_'+str(current_dateTime)+'/mask_'+str(basename(input_image)).split('.')[0]+'.tif', 
                         labeled_image_mask)
...
st.title("Segmentation of Mitochondria")

input_image = st.text_input("Please enter the path to the image to be segmented", "")
...
if st.button("Upload file"):
    ...
    # Now do something with the image! For example, let's display it:
    st.image(crop_image_out, channels="BGR")  
